...
- 2026-09-13 (M006): A sentence about what intraclass as a whole takes must hold for all three exported functions. `choose_icc()` has no `data` argument, so "one rating per row" is true only of `icc()`. Name the function a claim is about.
