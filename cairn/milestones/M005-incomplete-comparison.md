...
**In:** 14 new rows in `analysis/comparison.R` and its results file, which replace the two interaction-model irrICC rows. State of the field sentences say which coefficients psych and intraclass agree or differ on for `ratings_incomplete`. They give each package's divisor for the averaged coefficients. The irrICC comparison sentence names the model without interaction. The sentence "On the `ratings_incomplete` example, psych reported 6 subjects." is removed.
